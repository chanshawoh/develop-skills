# Project Docs Concurrent Edit Safety

Use when editing shared task/spec/report documents that may be touched by another agent, sibling worktree, or external documentation system.
...
When the user says a local documentation surface is authoritative and external trackers do not matter, do not block on Linear/API sync. For a Linear-synced local task note, update all visible local status surfaces consistently:

- frontmatter `status` and `status_type`
- metadata table status row
- bottom status checkbox/label
- orchestration task status under the selected project-docs task area when present, for example `AI编排/Tasks/`
...
Phrase the result as a local project-docs status update, not an external Linear update.

## Why

Avoids overwriting concurrent orchestration edits, especially status flips in durable task notes, while respecting the user's preferred documentation surface.
