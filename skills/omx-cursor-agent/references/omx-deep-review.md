# OMX Deep Review After Implementation

Use when the user asks OMX to "deep review", "deep interaction", "再跑一遍", "看看遗漏", or "可重构的地方" after an implementation/commit.

## Purpose

This is not another implementation pass. It is a review/refactor-opportunity pass that should inspect the task/spec, previous verification, the committed diff, and key code paths, then write a durable report.
...
Prefer one-shot `omx exec` when the user asks from chat and does not need live back-and-forth:

```bash
omx exec --dangerously-bypass-approvals-and-sandbox --cd <repo> \
  "Run a deep interactive-style code review for <task-id> after commit <sha>. This is a review/refactor opportunity pass, not implementation. Read task note: <task-path>. Read previous verification: <verification-path>. Inspect commit diff with git show --stat and targeted git show. Focus on omissions, semantic gaps, bugs, over-complexity, refactor opportunities, and missing tests. Pay special attention to: <risk areas>. Do not modify source code. Write a concise Markdown report to: <report-path>. Required sections: Verdict, Must Fix, Should Improve, Refactor Opportunities, Test Gaps, Release Recommendation, Exact Commands Reviewed. Use bullets, no markdown tables."
```

Use interactive `omx --madmax --high` only when the user explicitly wants to participate in the dialogue or make decisions mid-review.
...
- `Exact Commands Reviewed`: commands OMX ran or reviewed.
...
- Avoid Markdown tables in Telegram-facing summaries and durable reports that will be relayed through Telegram; bullets survive formatting better.
- If the user asks for "deep interaction" from chat, one-shot `omx exec` with an interactive-style prompt is acceptable unless they explicitly need live TUI participation.
