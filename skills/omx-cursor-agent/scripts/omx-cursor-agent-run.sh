...
usage() {
  cat <<'EOF'
Usage:
  omx-cursor-agent-run.sh --repo /path/to/repo --task task-id --prompt-file /tmp/task.cursor.prompt.md [--fresh] [--model model] [--mode ask|plan] [--no-approve-mcps] [--worktree name]

Non-interactive Cursor headless launcher. Stores session state under /tmp/omx-cursor-agent-sessions.
...
state_dir="/tmp/omx-cursor-agent-sessions/$slug"
mkdir -p "$state_dir"
...
echo "omx-cursor-agent log: $log_file" >&2
echo "omx-cursor-agent last output: $last_file" >&2
